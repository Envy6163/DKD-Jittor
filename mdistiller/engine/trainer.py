...
class BaseTrainer(object):
    ...
    def train(self, resume=False):
        ...
        if resume:
            ...
            epoch = state["epoch"] + 1
            self.distiller.load_parameters(state["model"])
            self.optimizer.load_state_dict(state["optimizer"])
            self.best_acc = state["best_acc"]
        while epoch < self.cfg.SOLVER.EPOCHS + 1:
            self.train_epoch(epoch)
            epoch += 1
        print(log_msg(f"Best accuracy:{self.best_acc}", "EVAL"))
        with open(os.path.join(self.log_path, "worklog.txt"), "a") as writer:
    ...
    def train_epoch(self, epoch):
        ...
        if epoch % self.cfg.LOG.SAVE_CHECKPOINT_FREQ == 0:
            save_checkpoint(state, os.path.join(self.log_path, f"epoch_{epoch}"))
            save_checkpoint(
                student_state, os.path.join(self.log_path, f"student_{epoch}")
            )
    ...
    def train_iter(self, data, epoch, train_meters):
        ...
        train_meters["losses"].update(float(loss), batch_size)
        train_meters["top1"].update(float(acc1[0]), batch_size)
        train_meters["top5"].update(float(acc5[0]), batch_size)
